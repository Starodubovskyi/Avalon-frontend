...
const VesselsFilters: React.FC<VesselsFiltersProps> = ({
  ...
  useEffect(() => {
    if (popularRef.current) {
      setMaxHeight((prev) => ({
        ...prev,
        popular: openSections.popular ? popularRef.current!.scrollHeight : 0,
      }));
    }
    if (basicRef.current) {
      setMaxHeight((prev) => ({
        ...prev,
        basic: openSections.basic ? basicRef.current!.scrollHeight : 0,
      }));
    }
    if (particularsRef.current) {
      setMaxHeight((prev) => ({
        ...prev,
        particulars: openSections.particulars
          ? particularsRef.current!.scrollHeight
          : 0,
      }));
    }
    if (voyageDataRef.current) {
      setMaxHeight((prev) => ({
        ...prev,
        voyageData: openSections.voyageData
          ? voyageDataRef.current!.scrollHeight
          : 0,
      }));
    }
  }, [openSections]);
        ...
        {Object.entries(filters).map(([section, { items, ref }]) => (
          <div key={section}>
            <button
              ...
              className="flex justify-between items-center w-full py-2 font-medium capitalize text-sm" // Adjusted font size
            >
              {section}
